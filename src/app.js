...
function rosterGroups(){
  ...
  return `<div class=roster-groups>${groupSlotsByCategory(purchased,false,state.aliasConfiguration.map(item=>item.alias)).map(({category,slots})=>`<section class=roster-group><h3>${escapeHtml(category)} <span>· ${slots.length}</span></h3><ul class=roster-players>${slots.map(slot=>{const player=raw.find(item=>item.id===slot.playerId),rosterPlayer=player??{id:slot.playerId,name:slot.playerId};return `<li><span class=roster-player-summary><b>${escapeHtml(rosterPlayer.name)}</b><span> · ${escapeHtml(player?.team||'—')}</span></span>${resetButton(rosterPlayer,true,'roster-player-reset')}</li>`}).join('')}</ul></section>`).join('')}</div>`;
}
...
const acquiredPlayerIds=()=>new Set(state.slots.map(slot=>slot.playerId).filter(Boolean));
