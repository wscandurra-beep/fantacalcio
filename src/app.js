...
import {auctionInjuryStatus,buildAuctionRows,moveAuctionPlayer} from './auction-view.js';
const loadIssues=[];
...
if(!Array.isArray(state.aliasConfiguration))state.aliasConfiguration=defaultAliasConfiguration(raw);
state.aliasConfiguration=validateAliasConfiguration(state.aliasConfiguration,mantraRoleOptions).configuration;
state.rosterSize=state.slots.length;
function players(){const classified=classifyPlayersByAliases(raw,state.aliasConfiguration);return state.aliasConfiguration.flatMap(({alias})=>assignTiers(rankPlayers(classified.filter(player=>player.strategicAlias===alias)),state.slots.filter(slot=>slot.category===alias).length));}
...
function rosterGroups(){
  ...
  return `<div class=roster-groups>${groupSlotsByCategory(purchased,false,state.aliasConfiguration.map(item=>item.alias)).map(({category,slots})=>`<section class=roster-group><h3>${escapeHtml(category)} <span>· ${slots.length}</span></h3><div class=roster-players>${slots.map(slot=>{const player=raw.find(item=>item.id===slot.playerId);return `<article class=roster-player><div><b>${escapeHtml(player?.name??slot.playerId)}</b><span>${escapeHtml(player?.team||'—')} · ${escapeHtml(player?.roles||slot.category)}</span></div><dl><div><dt>Slot</dt><dd>${escapeHtml(slot.id)}</dd></div><div><dt>Priorità</dt><dd>${escapeHtml(slot.priority)}</dd></div><div><dt>Prezzo</dt><dd>${escapeHtml(slot.actualPurchasePrice)} cr</dd></div></dl></article>`}).join('')}</div></section>`).join('')}</div>`;
}
...
function slotTable(slots,editable=false,priority=false){
  ...
  const body=editable?groupSlotsByCategory(slots,false,state.aliasConfiguration.map(item=>item.alias)).map(({category,slots:grouped})=>{const variance=areaVariance(slots,category);return `<tr class="area-row"><th colspan="7">${escapeHtml(category)}</th><th>Δ Baseline <strong class="${varianceClass(variance)}">${signed(variance)}</strong></th></tr>${grouped.map(s=>slotRow(s,true)).join('')}`}).join(''):slots.map(s=>slotRow(s,false)).join('');
...
function auction(){
  const rows=buildAuctionRows(players(),state.slots,state.auctionView,true);
...
function aliasRow(item){const fixed=item.alias==='POR';return `<div class="alias-row" data-alias-row><label>Alias<input data-alias-name value="${escapeHtml(item.alias)}" ${fixed?'disabled':''}></label><label>Slot<input data-alias-slots type=number min="${fixed?3:0}" max="${fixed?3:31}" value="${item.slotCount}" ${fixed?'disabled':''}></label><label>Mantra Roles<select data-alias-roles multiple size=5 ${fixed?'disabled':''}>${mantraRoleOptions.map(role=>`<option value="${escapeHtml(role)}"${item.mantraRoles.includes(role)?' selected':''}>${escapeHtml(role)}</option>`).join('')}</select></label>${fixed?'<span class="pill">Fisso · Por</span>':'<button type=button class="action secondary remove-alias">Rimuovi</button>'}</div>`}
function slotCountControls(){return `<div class="slot-counts"><div class="slot-counts-head"><div><b>Slot per ruolo</b><span class=muted>Alias strategici · POR fisso a 3 · totale massimo 34</span></div><button type=button id=addAlias class="action secondary">+ Alias</button></div><div id=aliasRows>${state.aliasConfiguration.map(aliasRow).join('')}<div class="alias-row out-alias"><b>OUT</b><span>Nessuno slot · Tier unico</span><span class=muted>Tutti i ruoli Mantra · override manuale</span></div></div></div>`}
...
function initBaselineEditor(){
  ...
  const bindRemove=()=>document.querySelectorAll('.remove-alias').forEach(button=>button.onclick=()=>button.closest('[data-alias-row]').remove());bindRemove();
  document.querySelector('#addAlias').onclick=()=>{const holder=document.querySelector('#aliasRows'),wrapper=document.createElement('div');wrapper.innerHTML=aliasRow({alias:'NUOVO',slotCount:0,mantraRoles:[]});holder.insertBefore(wrapper.firstElementChild,holder.querySelector('.out-alias'));bindRemove()};
  let drag=null;
  const fillTo=target=>{
