...
function resetButton(player,enabled,extraClass=''){
  return `<button type=button class="player-reset ${extraClass}" onclick="resetOne('${player.id}')" aria-label="Annulla vendita o acquisto di ${escapeHtml(player.name)}" title="Ripristina giocatore"${enabled?'':' disabled'}><span aria-hidden=true>↶</span></button>`;
}
function auctionCard(player,index,acquiredPlayerIds){
  const storedStatus=state.market[player.id]?.marketStatus||'AVAILABLE',acquired=storedStatus==='MY TEAM'||acquiredPlayerIds.has(player.id),marketStatus=storedStatus==='SOLD'?'SOLD':acquired?'MY TEAM':'AVAILABLE';
