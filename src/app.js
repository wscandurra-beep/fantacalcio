...
function resetButton(player,enabled,extraClass=''){
  if(!enabled)return '';
  return `<button type=button class="player-reset ${extraClass}" onclick="resetOne('${player.id}')" aria-label="Annulla vendita o acquisto di ${escapeHtml(player.name)}" title="Ripristina giocatore"><span aria-hidden=true>↶</span></button>`;
}
