...
function rosterGroups(){
  ...
  return `<div class=roster-groups>${groupSlotsByCategory(purchased).map(({category,slots})=>`<section class=roster-group><h3>${escapeHtml(slotCategoryLabel(category))} <span>· ${slots.length}</span></h3><div class=roster-players>${slots.map(slot=>{const player=raw.find(item=>item.id===slot.playerId);return `<div class=roster-player><b>${escapeHtml(player?.name??slot.playerId)}</b><span>· ${escapeHtml(player?.team||'—')}</span></div>`}).join('')}</div></section>`).join('')}</div>`;
}
function cockpit(){const b=budgetSummary(state.budget,state.slots),ps=players(),purchased=state.slots.filter(s=>s.playerId);return `<div class=eyebrow>Live strategy</div><h1>Buonasera, Mister.</h1><div class=grid>${[['Budget iniziale',b.budget],['Speso',b.spent],['Rimanente',b.remaining],['Varianza',b.variance]].map((x,i)=>`<div class="card metric"><label>${x[0]}</label><strong class=${i===2?'lime':''}>${x[1]} <small>cr</small></strong></div>`).join('')}<div class="card wide roster-card"><div class=section-head><h2>Rosa</h2><span class=pill>${purchased.length} / ${state.slots.length}</span></div>${rosterGroups()}</div>${marketPressure(ps)}</div><div class=section><div class=section-head><h2>Slot prioritari</h2><button class=action onclick="goMarket()">Apri mercato</button></div>${slotTable(state.slots,false,true)}</div>`}
...
function slotTable(slots,editable=false,priority=false){
  ...
  const body=editable?groupSlotsByCategory(slots).map(({category,slots:grouped})=>{const variance=areaVariance(slots,category);return `<tr class="area-row"><th colspan="7">${slotCategoryLabel(category)}</th><th>Δ Baseline <strong class="${varianceClass(variance)}">${signed(variance)}</strong></th></tr>${grouped.map(s=>slotRow(s,true)).join('')}`}).join(''):slots.map(s=>slotRow(s,false)).join('');
