...
function prolog(){const runs=proControLog.runs,latest=runs[0];if(!latest)return `<div class=eyebrow>Diagnostica scraper</div><h1>PRO/CONTRO Log</h1><div class=notice>Nessuna esecuzione registrata.</div>`;const metrics=[['Status',latest.status],['Scraped',latest.scrapedRecords],['Matched',latest.matched],['Unmatched',latest.unmatched],['Ambiguous',latest.ambiguous],['Coverage',`${latest.coveragePct}%`]],details=run=>[...(run.errors||[]).map(x=>`Errore · ${x.name||x.id||''} ${x.error||x}`),...(run.unmatchedRecords||[]).map(x=>`Unmatched · ${x.name} · ${x.team||'—'}`),...(run.ambiguousRecords||[]).map(x=>`Ambiguous · ${x.name} · candidati ${(x.candidateIds||[]).join(', ')}`)];return `<div class=eyebrow>Diagnostica scraper</div><h1>PRO/CONTRO Log</h1><div class="grid log-summary">${metrics.map(([k,v])=>`<div class="card metric"><label>${k}</label><strong>${escapeHtml(v??'—')}</strong></div>`).join('')}</div><section class="card section log-history"><h2>Storico esecuzioni</h2>${runs.map(run=>{const issues=details(run);return `<details><summary><b>${escapeHtml(run.runTimestamp)}</b> · ${escapeHtml(run.status)} · ${run.scrapedRecords} scraped · ${run.coveragePct}%</summary><p class=log-metrics>Source ${run.sourcePlayers} · discovered ${run.playersDiscovered} · processed ${run.playersProcessed} · requests OK ${run.requestsSuccessful} · failed ${run.requestsFailed} · PRO ${run.withPro} · CONTRO ${run.withContro} · both ${run.withBoth} · duplicates ${run.duplicates}</p>${issues.length?`<ul>${issues.map(x=>`<li>${escapeHtml(x)}</li>`).join('')}</ul>`:'<p class=muted>Nessun errore di scraping o matching.</p>'}</details>`}).join('')}</section>`}
...
function render(){document.querySelector('header i').textContent=state.started?'ASTA LIVE':'STRATEGIA';const warning=loadIssues.length?`<div class="notice load-warning" role="alert"><b>Dati caricati solo in parte.</b> ${loadIssues.join(' · ')}. Puoi comunque aprire l’app e riprovare aggiornando la pagina.</div>`:'';document.querySelector('#app').innerHTML=warning+({cockpit,market,strategy,quality,prolog})[view]();if(view==='quality')document.querySelector('#refreshInjuries').onclick=refreshInjuries;if(view==='strategy'){document.querySelector('#formation').onchange=event=>document.querySelector('#formationSummary').innerHTML=formationSummary(event.target.value);initBaselineEditor()}if(view==='market')initMarketFilters()}
