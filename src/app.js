...
let [raw,injuryUpdate]=await Promise.all([
...
const pressureCards=items=>items.map(item=>`<div class=tier><span class=muted>${item.label}</span><b>${item.available} / ${item.total}</b><div class=bar><i style="width:${item.total?100*item.available/item.total:0}%"></i></div></div>`).join('');
function marketPressure(ps){
  const tierItems=tierDepletion(ps,state.market).slice(0,12).map(item=>({...item,label:`${item.category} · Tier ${item.tier}`}));
  const mantraItems=mantraRoleDepletion(raw,state.market).map(item=>({...item,label:item.roles}));
  const items=pressurePage==='tier'?tierItems:mantraItems;
  return `<div class="card wide market-pressure"><div class=section-head><h2>Pressione mercato</h2><span class=muted>Disponibili / originali</span></div><div class=pressure-tabs role=tablist aria-label="Vista pressione mercato"><button type=button role=tab aria-selected="${pressurePage==='tier'}" class="${pressurePage==='tier'?'active':''}" onclick="setPressurePage('tier')">Tier</button><button type=button role=tab aria-selected="${pressurePage==='mantra'}" class="${pressurePage==='mantra'?'active':''}" onclick="setPressurePage('mantra')">Ruoli Mantra</button></div><div class=pressure>${pressureCards(items)}</div></div>`;
}
